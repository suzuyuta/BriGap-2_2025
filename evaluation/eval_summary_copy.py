# This is copy and modification of summary.py
...
SUMMARY_TABLE = [
    ...
    'Average crossing', 'No crossing', 'Tagging accuracy', 'Height of sentence'
]
...
SUMMARY_NAME_TABLE = [
    ...
    'average_crossing', 'no_crossing', 'tagging_accuracy', 'sent_height'
]
...
class Result:
    ...
    STATISTICS_TABLE = [
        ...
        'cross_brackets', 'words', 'correct_tags', 'tag_accracy', 'height', 'snt_f1', 'unmatch'
    ]
    ...
    def get_statistic(self, name):
        return self._staticis.get(name, None)
...
def tree_height(tree_string):
    tree = Tree_.fromstring(tree_string)
    height = tree.height()
    return height

def max_tree_depth(tree):
    # tree = Tree_.fromstring(tree_string)
    if isinstance(tree, Tree_):
        if len(tree) == 0:
            return 1
        else:
            child_depths = [max_tree_depth(child) for child in tree]
            return int(1 + max(child_depths))
    else:
        return 0
...
def summary(results):
    ...
    summay_list[11] = [item.height for item in correct_results]
